...
//
// Mostly based on s6e3fa2 and s6e63m0. Table values taken from Samsung's
// android kernel
...
static int ea8061v_ams497ee01_get_brightness(struct backlight_device *bl_dev)
{
...
{
	dsi_dcs_write_seq(ctx->dsi, 0xf7, 0x01);
	return 0;
}
...
static int ea8061v_ams497ee01_set_brightness(struct backlight_device *bl_dev)
{
	...
	if (brightness < EA8061V_AMS497EE01_MIN_BRIGHTNESS ||
		brightness > bl_dev->props.max_brightness) {
		dev_err(dev, "Invalid brightness: %u\n", brightness);
		return -EINVAL;
	}

	// FIXME: do we need to unlock?
	// /* unlock f0 */
...
static const struct backlight_ops ea8061v_ams497ee01_bl_ops = {
	.get_brightness = ea8061v_ams497ee01_get_brightness,
...
static int ea8061v_ams497ee01_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->bl_dev = backlight_device_register("ea8061v_ams497ee01", dev, ctx,
						&ea8061v_ams497ee01_bl_ops, NULL);
